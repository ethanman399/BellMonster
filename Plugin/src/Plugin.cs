...
namespace BellMonster {
    ...
    public class Plugin : BaseUnityPlugin {
        ...
        private void Awake() {
            ...
            var bundleName = "bellmonsterassets";
            ModAssets = AssetBundle.LoadFromFile(Path.Combine(Path.GetDirectoryName(Info.Location), bundleName));
            if (ModAssets == null) {
                Logger.LogError($"Failed to load Bell Monster assets.");
                return;
            }
            else
            {
                Logger.LogInfo($"Successfully loaded Bell Monster assets.");
            }

            // We load our assets from our asset bundle. Remember to rename them both here and in our Unity project.
            var BellMonster = ModAssets.LoadAsset<EnemyType>("BellMonsterObj");
            var BellMonsterTN = ModAssets.LoadAsset<TerminalNode>("BellMonsterTN");
            var BellMonsterTK = ModAssets.LoadAsset<TerminalKeyword>("BellMonsterTK");
            bellStep = ModAssets.LoadAsset<AudioClip>("BellStep.ogg");
            if (bellStep == null) {
                Logger.LogError($"Failed to load BellStep.ogg asset.");
                return;
            }

            // Optionally, we can list which levels we want to add our enemy to, while also specifying the spawn weight for each.
            /*
            var BellMonsterLevelRarities = new Dictionary<Levels.LevelTypes, int> {
            ...
            var BellMonsterCustomLevelRarities = new Dictionary<string, int> {
                {"EGyptLevel", 50},
            ...
            */
            ...
            NetworkPrefabs.RegisterNetworkPrefab(BellMonster.enemyPrefab);
            ...
            Enemies.RegisterEnemy(BellMonster, BoundConfig.SpawnWeight.Value, Levels.LevelTypes.All, BellMonsterTN, BellMonsterTK);
            // For using our rarity tables, we can use the following:
            // Enemies.RegisterEnemy(BellMonster, BellMonsterLevelRarities, BellMonsterCustomLevelRarities, BellMonsterTN, BellMonsterTK);
